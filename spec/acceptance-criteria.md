...
| ID   | Criterion                                                                                                                            |
|------|--------------------------------------------------------------------------------------------------------------------------------------|
| B-01 | Responses are written in first person as the portfolio owner, using a professional and concise tone                                  |
| B-02 | Each response answers the current request `question`, not a previous question in the conversation                                    |
| B-03 | Follow-up questions use relevant completed conversation history when available                                                       |
| B-04 | If the required information is unavailable in resume, interests, or completed history, respond: `"I don't have that information."`   |
| B-05 | Questions unrelated to recruiting for a professional role are rejected with `400 INVALID_QUESTION`                                   |
...
| C-02 | Prompt context uses the most recent completed turns first when history exceeds limits                                       |
...
| W-02 | The persisted question record is written directly with `status: complete` and the final `answer`; no separate status update is required                                         |
...
| E-02 | An OpenAI `5xx` response results in a `502 UPSTREAM_ERROR` response to the client                                       |
...
| S-04 | API Gateway execution logging must not record request or response bodies |
